...
class Filter < Instance
  ...
  def initialize
    ...
    @queue = []
    @count = 0
    @graph_point = GraphPoint.first_or_create(:analysis_metadata_id => 0, :label => "skip_count_sample", :graph_id => 0, :curation_id => 0)
  ...
  def collect
    ...
    begin
      ...
      TweetStream.configure do |config|
        config.consumer_key = "RBY3pMQVAYbHW3Eld1DUg" #oauth_settings[account]["oauth_settings"]["consumer_key"]
        config.consumer_secret = "SyaJ6osRIQb3O4vsKsnNJoI31tWAhBYhgCQYS04" #oauth_settings[account]["oauth_settings"]["consumer_secret"]
        config.oauth_token = "148584489-8qB2SzclXvglJcuCKe5JdqDX1fruGa12ftnQLBJj" #oauth_settings[account]["access_token"]["access_token"]
        config.oauth_token_secret = "FobF7iibpFGU7HtYlfsR7fKR2q06R9fjsbSygbm2cU" #oauth_settings[account]["access_token"]["access_token_secret"]
        config.auth_method = :oauth
        config.parser   = :yajl
      end
      client.on_interval(CHECK_FOR_NEW_DATASETS_INTERVAL) { rsync_previous_files; @start_time = Time.now; puts "Switching to new files..."}
  ...
  def determine_datasets(tweet)
    ...
    params.each_pair do |method, values|
      if method == "locations"
        ...
        values.each do |value|
          valid_datasets << value[:dataset_id]
        end
      end
    end
